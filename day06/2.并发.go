...
// 博客地址: https://www.liwenzhou.com/posts/Go/concurrence/
...
func randInt64(jobChan chan<- int64) {
	...
	for i := 0; i < 10; i++ {
		jobChan <- int64(rand.Int())
	}
	close(jobChan)
}

func countIn64(jobChan <-chan int64, resultChan chan<- int64) {
	defer wg.Done()
	var count int
	for i := range jobChan {
		iStr := strconv.Itoa(int(i))
		for i := 0; i < len(iStr); i++ {
			ret, _ := strconv.Atoi(string(iStr[i]))
			count += ret
		}
		resultChan <- int64(count)
		fmt.Printf("%s的结果为%d\n", iStr, count)
		count = 0
	}
}
...
func main() {
	...
	wg.Add(1)
	go randInt64(jobChan)
	for i := 0; i < 25; i++ {
		wg.Add(1)
		go countIn64(jobChan, resultChan)
	}
	wg.Wait()
	close(resultChan)
