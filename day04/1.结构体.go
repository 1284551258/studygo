...
// 博客地址：https://www.liwenzhou.com/posts/Go/10-struct/
...
type dog struct {
	feet uint8
	animal //不能使用animal animal 只能使用匿名嵌套
}
...
type persion struct {
	ID int `json:"id"`
	Sex string //tag默认为字段名
	city string //小写的json包获取不到
}
...
func main(){
	...
	var j = `{"ID":2,"Sex":"男","city":"广州"}`
	var p2 persion
	err2 := json.Unmarshal([]byte(j), &p2)
	if err2 != nil {
		fmt.Printf("反序列化失败！")
		return 
	}
