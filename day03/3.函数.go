...
// 博客地址：https://www.liwenzhou.com/posts/Go/09_function/
/*
...
func main() {
	a := func(f func(x, y int), m, n int) func() {
		return func() {
			f(m, n)
		}
	}(f2, 100, 100)
...
func s1(s string) func(n string) string {
...
func main() {
	jpgFunc := s1(".jpg")
	txtFunc := s1(".txt")
	fmt.Println(jpgFunc("p1.jpg"))
	fmt.Println(txtFunc("t1"))
}


func ad(base int) (func(int) int, func(int) int) {
	base1 := base
	add := func(a int) int {
		base1 += a
		return base1
...
func main3() {
	fmt.Println(distribution, len(distribution))
	left := dispatchCoin()
	fmt.Println("剩下：", left)
}
*/
